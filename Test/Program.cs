...
namespace Test
{
    ...
    //{
    //    int x = 1;
    //    int y;//字段是有默认值的,int默认值为0,引用类型默认值为null
    //    public B()
    //    {
    //        y = -1;
    //    }
    ...
    //静态构造函数:只会被执行一次,也就是创建第一个实例或引用任何静态成员之前,又.NET自动调用
    class A {
        public static int X;//默认值为0
        static A() {
            X = B.Y + 1;//2
        }
    }
    class B {
        ...
        static void Main() {
            Console.WriteLine("X={0},Y={1}", A.X, B.Y);//4
            Console.ReadLine();
        }
    }
